...
termux_step_pre_configure() {
	...
	TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -DNUMPY_INCLUDE_DIR=$TERMUX_PREFIX/lib/python$TERMUX_PYTHON_VERSION/site-packages/numpy/_core/include"
	TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -Donnxruntime_BUILD_SHARED_LIB=ON"
	TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -DCMAKE_BUILD_TYPE=Release "
	#TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -Donnxruntime_GCC_STATIC_CPP_RUNTIME=ON "
	TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -Donnxruntime_ENABLE_PYTHON=ON "
	TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -Donnxruntime_DEV_MODE=OFF "
	#TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -Donnxruntime_USE_NNAPI_BUILTIN=ON "
	#TERMUX_PKG_EXTRA_CONFIGURE_ARGS+=" -Donnxruntime_EXTENDED_MINIMAL_BUILD=ON "
